...
$(document).ready(function () {
    ...
    if ($('.registration-container').length) {
        ...
        if ($('.stage_2').length) {
            ...
            // device counter
            $('.device_counter #counter_container')
                .on('click', 'a:first-child', function() {
                    // increase
                    updateDeviceNumber('inc');
                })
                .on('click', 'a:last-child', function() {
                    // decrease
                    updateDeviceNumber('dec');
                })
            ;

            $('.device_counter')
                .on('click', '#num_devices', function(e) {
                    // toggle device # input/static
                    ...
                    if ($(this).hasClass('editable')) {
                        // input status - change to static

                        toggleDeviceCount('toStatic');
                    } else {
                        toggleDeviceCount('toInput');
                    }
                })
                .on('change focusout', '#edit_num_devices', function() {
                    toggleDeviceCount('toStatic');
                })
            ;
        ...
        $('.signup-form').submit(function() {
            ...
            switch (current_step) {
                case '1':
                    // trying this a different way fbq('track', 'InitiateCheckout');
                    // step 1 - Account
                ...
                case '3':
                    // trying this a different way fbq('track', 'AddPaymentInfo');
                    // step 3 - Review
    ...
    */
    ...
    if ($('.userdevices_container').length) {
        ...
        $('.dialog_panels')
            ...
            //-- user actions
            .on('click', '#btn_confirm_remove_user', function() {
                // confirm remove user
