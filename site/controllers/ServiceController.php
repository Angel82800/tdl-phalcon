...
class ServiceController extends ControllerBase
{
    ...
    public function billingAction()
    {
        ...
        if ($this->request->isPost()) {
            ...
            if (! count($messages)) {
                ...
                $plan = $this->billingHelper->getPlanInfo('device', false, $device_count);
                ...
                if ($organization->update() === false) {
                    ...
                    // $pricingHelper->setPrice($billing_data['num_devices'] * $plan->amount / 100);
                    ...
                    // subscribe customer with coupon
                    $subscription = $stripe->subscribeCustomer($organization->stripe_customer_id, $plans, $coupon, $plan->trial_period_days);

                    $this->logger->info('[SERVICE] Subscribed user ' . $user_id . ' with ' . $device_count . ' device(s) to stripe plan ' . $plan->id);
                    ...
                    if ($subscription) {
                        ...
                        if ($coupon) $stripeSubscriptionData['coupon'] = $coupon;

                        if ($stripeSubscription->create($stripeSubscriptionData) === false) {
                            $stripe_error = implode(', ', $stripeSubscription->getMessages());
                            $this->logger->error('[SERVICE] Error while logging stripe subscription : ' . $stripe_error);
                            $errorMessage = 'Error while subscribing customer: ' . $stripe_error;
                        } else {
                            $this->logger->info('[SERVICE] Created ent_stripe_subscription entry');
                            ...
                            $upcoming_invoice = $stripe->getUpcomingInvoice($organization->stripe_customer_id);

                            $data['thankyou'] = true;
                            $data['final_price'] = '$' . number_format($upcoming_invoice->amount_due / 100, 2) . ' p/m';
                            // $data['final_price'] = '$' . number_format($pricingHelper->getPricing('final'), 2) . ' p/m';

                            // $data['trial_end'] = date('F jS', strtotime('+15 days'));
                            $data['trial_end'] = date('F jS', strtotime('+' . ($plan->trial_period_days ? $plan->trial_period_days : 0) . ' days'));

                            $this->session->set('thankyou_billing_data', $data);
                            ...
                            $emailData = [
                                ...
                                'final_price'   => '$' . number_format($upcoming_invoice->amount_due / 100, 2),
                                'card_brand'    => $customer->sources->data[0]->brand,
                                'card_last4'    => $customer->sources->data[0]->last4,
                            ];
                            $this->mail->sendMail($user->pk_id, 'confirmation', $emailData);
        ...
        $trial_end = date('F jS', strtotime('+' . $this->billingHelper->getPlanInfo('device', 'trial_period_days') . ' days'));
        ...
        $data = array_merge($data, [
            ...
            'plan_price'    => '$' . number_format($this->billingHelper->getPlanInfo('device', 'amount') / 100 ,2),
            'is_free'       => ($this->billingHelper->getPlanInfo('device', 'amount') == 0),
        ]);
    ...
    public function deviceAction()
    {
        ...
        if ($organization->stripe_customer_id) {
            $this->persistent->current_stripe_plan = $this->billingHelper->getSubscriptionForType($organization, 'device');
        ...
        // store device plan info to reduce request time
        $this->persistent->device_plan = $this->billingHelper->getPlanInfo('device');
    ...
    public function manageDeviceAction()
    {
        ...
        if ($this->request->isPost() && $this->request->isAjax()) {
            ...
            if ($type == 'getSummary') {
                if ($this->session->get('is_ftu')) {
                    $content['is_ftu'] = true;
                    $content['trial_end'] = date('F jS', strtotime('+' . $this->persistent->device_plan->trial_period_days . ' days'));

                    $device_plan = $this->settings['device'];

                    $content['plan_price'] = '$' . number_format($this->persistent->device_plan->amount / 100 ,2);
                    ...
                    // $pricingHelper->setPrice($this->persistent->current_stripe_plan-)
                    ...
                    $upcoming_invoice = $stripe->getUpcomingInvoice($organization->stripe_customer_id, $coupon_id, $subscription_id,  $subscription_items);
                ...
                if (! $is_valid_request) {
                    ...
                    if ($update_result['status'] != 'success') {
                        ...
                        //- get details to show in summary
                        ...
                        $change_label .= ($device_change == 1) ? ' Device' : ' Devices';
                        ...
                        if ($this->persistent->current_stripe_plan->discount) {
                            ...
                            $order_details['discount'] = '- $' . number_format($pricingHelper->getPricing('discounted'), 2) . ' p/m';
                        }
