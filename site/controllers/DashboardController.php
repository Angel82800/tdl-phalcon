...
class DashboardController extends ControllerBase
{
    ...
    public function indexAction()
    {
        ...
        $org_total_devices = $org_used_devices = 0;

        if ($user->role->name != 'user') {
            $org_devices = $this->stats->getOrgDevicesPerUser($user->organization->pk_id);

            $org_total_devices = 0;
            $org_used_devices = 0;
            foreach ($org_devices as $user_device) {
                $org_total_devices += $user_device['device_count'];
                $org_used_devices += $user_device['used_device_count'];
            }
        }

        if (! $device_count['total_count'] && ! $org_used_devices) {
            // no devices for user

            $data = [
                'trial_days' => $this->billingHelper->getPlanInfo('device', 'trial_period_days'),
            ];

            $this->view->pick('dashboard/nothing');
            $this->view->setVars($data);
        } else if (! $device_count['used_count'] && $device_count['total_count'] && ! $org_used_devices) {
    ...
    public function getstatsAction()
    {
        ...
        if ($this->request->isPost() && $this->request->isAjax()) {
            ...
            if ($type == 'data_protected') {
                ...
                if ($user->role->name == 'user') {
                    // load stats for user only
                    $blocked_threats = $this->stats->blockedThreats($user_id);
                } else {
                    // load stats for organization
                    $blocked_threats = $this->stats->blockedThreats($organization->pk_id, [ 'type' => 'organization' ]);
                }
            ...
            } else if ($type == 'threat_indicators') {
            ...
            } else if ($type == 'your_devices') {
                //--- your devices ---
                // $device_status = $this->stats->deviceSummary($user->GUID);
                ...
                if ($user->role->name == 'user') {
                    ...
                    $device_status = $this->stats->deviceSummary($organization->pk_id, [ 'type' => 'organization' ]);
                }
                ...
                foreach ($device_status as $status) {
                    ...
                    if (! $status['datetime_disconnected'] && $status['datetime_connected']) {
                        // connected
                        $status_html .= '<div class="element"><i class="text-green text-4em i-' . $status["device_type"] . '"></i></div>';
                    } else {
                        // disconnected
                        $status_html .= '<div class="element"><i class="text-mid-grey text-4em i-' . $status["device_type"] . '"></i></div>';
                    }
                    $status_html .= '<div class="element pl-1">';

                    $status_html .= '<p class="header">' . $status['user_device_name'] . '</p>';

                    if (! $status['datetime_disconnected'] && $status['datetime_connected']) {
                        $status_html .= '<p class="sub_header hide-for-small-only">Currently Connected | ' . $this->stats->formatBytes($status['protected_data'], 1) . ' Data Protected All Time</p>';
                    } else {
