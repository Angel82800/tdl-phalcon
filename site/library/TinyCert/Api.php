<?php
	...
	class Api extends Component
	{
		...
		public function __construct ()
		{
			...
			$this->apikey = $config->tinycert->key;
			...
			if ($resp)
			{
				...
				if (preg_match ('#^HTTP/\d\.\d (\d+) #', $headers, $m))
				{
					...
					if ($status === 200)
					{
						...
						error_log("TinyCert API Error: $status, $body");
						$this->error = array
						(
							'status' => $status,
							'code' => $data ['code'],
		...
		{
			$config = $this->getDi()->get('config');
			$resp = $this->do_request ('/connect', array ('email' => $config->tinycert->email, 'passphrase' => $config->tinycert->password));
			if ($resp)
			{
